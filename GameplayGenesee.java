...
public class GameplayGenesee 
{

	//your first shot is 1
	int shotno =1;
	int holeno;
	//creates a holes class which has info for each hole for Genesee
	GeneseeHoles hole = new GeneseeHoles();

	public GameplayGenesee(int holeno) {
		this.holeno = holeno;
	}
	...
	public int shot() {
		...
		System.out.println("The par for this hole is " + hole.getPar(holeno));
		...
		while (true) {
			...
			if (Math.abs(currentdis) > 20) 
			{
				...
				System.out.println("This is shot number: " + shotno);
